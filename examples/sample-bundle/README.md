...
jq '(.output_files) |= map(select(.kind != "image" and (.kind | startswith("handoff_") | not))) | (.next_steps) |= map(select(contains("handoff/") | not))' examples/sample-bundle/output/manifest.json > /tmp/technical-visualizer-sample-manifest.json
