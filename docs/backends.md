...
`openai` calls the OpenAI Images API with `gpt-image-2`. It requires `OPENAI_API_KEY`. It sends source-derived prompt content and scaffold HTML to OpenAI, writes the returned image to `final.png`, and fails if the remote generation call fails.
...
`hybrid` follows the same fallback behavior as `auto`: it tries OpenAI when credentials exist, then falls back to the local preview with a warning if OpenAI cannot produce an image.
